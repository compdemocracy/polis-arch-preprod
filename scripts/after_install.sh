...
PRE_CONFIGURED_ENV=$(aws secretsmanager get-secret-value --secret-id preprod-polis-web-app-env-vars --query SecretString --output text --region us-east-1)


if [ -z "$PRE_CONFIGURED_ENV" ]; then
  echo "Error: Could not retrieve pre-configured .env from SSM Parameter preprod-polis-web-app-env-vars"
  exit 1
fi
...
SECRET_ARN=$(aws ssm get-parameter --name /preprod/polis/db-secret-arn --query 'Parameter.Value' --output text --region us-east-1)

if [ -z "$SECRET_ARN" ]; then
  echo "Error: Could not retrieve DB Secret ARN from SSM Parameter /preprod/polis/db-secret-arn"
  exit 1
fi
...
DB_HOST=$(aws ssm get-parameter --name "/preprod/polis/db-host" --query 'Parameter.Value' --output text --region us-east-1)
DB_PORT=$(aws ssm get-parameter --name "/preprod/polis/db-port" --query 'Parameter.Value' --output text --region us-east-1)
...
yes | docker system prune -a --filter "until=72h"
